    ...
    C: Has<XSpanIdString> + Clone + Send + Sync + 'static,
{
    ...
    async fn refresh_worker(
        &self,
        param_refresh_worker_req: models::RefreshWorkerReq,
        context: &C,
    ) -> Result<RefreshWorkerResponse, ApiError> {
        let mut client_service = self.client_service.clone();
        let mut uri = format!("{}/v1/worker", self.base_path);
        ...
        // Body parameter
        let body = serde_json::to_string(&param_refresh_worker_req)
        ...
        *request.body_mut() = Body::from(body);
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::RefreshWorkerRsp>(body)?;
                Ok(RefreshWorkerResponse::RefreshWorkerResponse(body))
            }
            0 => {
                let body = response.into_body();
                let body = body
                ...
                Ok(RefreshWorkerResponse::UnexpectedError(body))
            }
            code => {
