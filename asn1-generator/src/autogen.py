...
import os
from render import generate_structs
...
def generate_file(protocol, outfile, infile, header, constants):
    i = input_filename(protocol, infile)
    out = output_filename(protocol, outfile)
    f = open(out, "w")
    f.write(AUTOGENERATED + i + "\n")
    f.write(header + "\n\n")
    f.write(generate_structs(input_file_path(protocol, i), constants))
    f.close()
...
def generate_rrc():
    protocol = 'rrc'
    constants = get_constants_from_file(
        input_file_path(protocol, input_filename(protocol, "All")))

    generate_file(protocol, "rrc.rs", "All", USE_ASN1 +
                  USE_BITSTRING + USE_NUM_ENUM, constants)
